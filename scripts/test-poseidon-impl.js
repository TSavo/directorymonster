#!/usr/bin/env node
/**
 * Test Poseidon Implementation
 * 
 * This script tests if different inputs produce different outputs in the Poseidon implementation.
 * 
 * Usage:
 *   node scripts/test-poseidon-impl.js
 */
...
const poseidonImplDir = path.join(circuitsDir, 'poseidon_impl');
const wasmPath = path.join(poseidonImplDir, 'poseidon_impl_js/poseidon_impl.wasm');
const zkeyPath = path.join(poseidonImplDir, 'poseidon_impl_final.zkey');
const tempDir = path.join(__dirname, '../temp');

// Check if the Poseidon implementation is set up
if (!fs.existsSync(wasmPath) || !fs.existsSync(zkeyPath)) {
  console.error('Poseidon implementation not set up. Please run `npm run compile:poseidon_impl` first.');
  process.exit(1);
}
...
const inputs = [
  ...
  {
    publicSalt: 789,
    username: 456, // Different username
    password: 456
  },
  {
    publicSalt: 456, // Different salt
    username: 123,
    password: 456
  }
];
