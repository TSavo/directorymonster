...
function extractDiff(diffContent) {
  ...
  if (diffBlocks && diffBlocks.length > 0) {
    ...
    const customDiffRegex = /FILE: (.+?)[\r\n]```typescript[\s\S]*?<<<<<<< SEARCH[\s\S]*?>>>>>>> REPLACE[\s\S]*?```/g;
    let customDiffMatch;
    
    while ((customDiffMatch = customDiffRegex.exec(diffText)) !== null) {
      const filePath = customDiffMatch[1];
      const customDiffBlock = customDiffMatch[0];
      
      // Extract the search and replace sections
      const searchStartIndex = customDiffBlock.indexOf('<<<<<<< SEARCH') + '<<<<<<< SEARCH'.length;
      const searchEndIndex = customDiffBlock.indexOf('=======', searchStartIndex);
      const replaceStartIndex = customDiffBlock.indexOf('=======', searchStartIndex) + '======='.length;
      const replaceEndIndex = customDiffBlock.indexOf('>>>>>>> REPLACE', replaceStartIndex);
      
      if (searchStartIndex !== -1 && searchEndIndex !== -1 && replaceStartIndex !== -1 && replaceEndIndex !== -1) {
        const searchText = customDiffBlock.substring(searchStartIndex, searchEndIndex).trim();
        const replaceText = customDiffBlock.substring(replaceStartIndex, replaceEndIndex).trim();
