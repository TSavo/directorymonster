...
describe('Data Retrieval User Flow', () => {
  ...
  it('should retrieve listings for a site', async () => {
    ...
    data.results.forEach((listing: Listing) => {
      expect(listing.siteId).toBe(site.id);
    });

    // Verify pagination metadata
    expect(data.pagination).toBeDefined();
    expect(data.pagination.totalResults).toBeGreaterThanOrEqual(data.results.length);
  });
  ...
  it('should follow the complete data retrieval flow', async () => {
    ...
    expect(listingsData.results).toBeDefined();
    expect(Array.isArray(listingsData.results)).toBe(true);

    // If there are listings for this category, verify they belong to both the site and the category
    if (listingsData.results.length > 0) {
      listingsData.results.forEach((listing: Listing) => {
        expect(listing.siteId).toBe(site.id);
        expect(listing.categoryId).toBe(category.id);
      });
    } else {
      // If no listings were found for this category, that's okay - just verify the pagination metadata
      expect(listingsData.pagination).toBeDefined();
      expect(listingsData.pagination.totalResults).toBe(0);
    }
  });
});
