...
describe('Cross-Site Data Isolation', () => {
  ...
  it('should maintain data isolation between sites when retrieving listings', async () => {
    ...
    listings1.forEach((listing: Listing) => {
    ...
    listings2.forEach((listing: Listing) => {
      expect(listing.siteId).toBe(site2.id);
    });
    
    // Verify no overlap between listings
    const site1ListingIds = listings1.map((l: Listing) => l.id);
    const site2ListingIds = listings2.map((l: Listing) => l.id);
  ...
  it('should filter search results by site', async () => {
    ...
    const request3 = createMockRequest(`/api/search?q=${commonKeyword}`);
    const response3 = await getSearch(request3);
    const data3 = await response3.json();
    
    // Verify results from both sites are included
    expect(data3.results.length).toBeGreaterThanOrEqual(2);
    
    // Verify results contain listings from both sites
    const site1Results = data3.results.filter((result: any) => result.siteId === site1.id);
    const site2Results = data3.results.filter((result: any) => result.siteId === site2.id);
    
    expect(site1Results.length).toBeGreaterThan(0);
  ...
  it('should resolve site identity correctly based on hostname', async () => {
    ...
    // Test with site 1 domain
    const request1 = createMockRequest('/api/site-info', {
      hostname: site1.domain,
    });
    
    // Call the site-info API endpoint
    const response1 = await getSiteInfo(request1);
    ...
    expect(data1.site.id).toBe(site1.id);
    expect(data1.site.slug).toBe(site1.slug);
    
    // Test with site 2 domain
    const request2 = createMockRequest('/api/site-info', {
      hostname: site2.domain,
    });
    
    // Call the site-info API endpoint
    const response2 = await getSiteInfo(request2);
    ...
    expect(data2.site.id).toBe(site2.id);
    expect(data2.site.slug).toBe(site2.slug);
  });
});
