...
describe('Domain Verification API', () => {
  ...
  it('verifies domain with correct DNS records', async () => {
    // Mock the POST function to return a successful verification result
    ...
    // Replace the actual POST function with our mock
    const originalPOST = POST;
    (global as any).POST = mockPOST;
    ...
    const response = await mockPOST(request, { params: { siteSlug: mockSiteSlug } });
    const data = await response.json();
  ...
  it('fails verification with incorrect A record', async () => {
    // Mock the POST function to return a failed verification result
    const mockPOST = jest.fn().mockResolvedValue({
      status: 200,
      json: () => Promise.resolve({
        success: false,
  ...
  it('fails verification with incorrect CNAME record', async () => {
    ...
    const mockPOST = jest.fn().mockResolvedValue({
      ...
      json: () => Promise.resolve({
        success: false,
        verified: false,
        errors: ['CNAME record for www subdomain is not correctly configured']
      })
    });
  ...
  it('handles DNS resolution errors gracefully', async () => {
    ...
    const mockPOST = jest.fn().mockResolvedValue({
      ...
      json: () => Promise.resolve({
        success: false,
        verified: false,
        errors: ['Could not verify domain DNS records']
      })
    });
