...
describe('ACL Cross-Tenant Detection', () => {
  ...
  /**
   * Test: detectCrossTenantAccess should return true for mixed tenant ACL
   */
  test('detectCrossTenantAccess returns true for mixed tenant ACL', () => {
    ...
    // Should return true (has entries for tenant2, which is cross-tenant access)
    expect(result).toBe(true);

    // Check for cross-tenant access for tenant2
    const result2 = detectCrossTenantAccess(acl, tenant2Id);

    // Should return true (has entries for tenant1, which is cross-tenant access)
