...
describe('Homepage Smoke Test', () => {
  ...
  test('Page loads and has proper title', async () => {
    ...
    // Check if we have a header element
    const hasHeader = await page.evaluate(() => {
      return document.querySelector('header') !== null ||
             document.querySelector('[data-testid="site-header"]') !== null ||
             document.querySelector('.header') !== null;
    });
    expect(hasHeader).toBe(true);
    
    // Check if we have a navigation element
    const hasNavigation = await page.evaluate(() => {
      return document.querySelector('nav') !== null ||
             document.querySelector('[data-testid="site-navigation"]') !== null ||
             document.querySelector('.navigation') !== null;
    });
    expect(hasNavigation).toBe(true);
    
    // Check if we have a footer element
    const hasFooter = await page.evaluate(() => {
      return document.querySelector('footer') !== null ||
             document.querySelector('[data-testid="site-footer"]') !== null ||
