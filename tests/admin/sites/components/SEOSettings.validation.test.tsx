...
describe('SEOSettings Validation', () => {
  ...
  it('validates structured data JSON format', async () => {
    ...
    // Add invalid JSON using fireEvent instead of userEvent to avoid keyboard parsing issues
    const structuredDataInput = screen.getByTestId('seoSettings-structuredData');
    fireEvent.change(structuredDataInput, {
      target: {
        value: '{invalid json}',
        name: 'structuredData'
      }
