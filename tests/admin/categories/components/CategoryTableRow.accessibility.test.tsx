...
describe('CategoryTableRow Component - Accessibility', () => {
  ...
  it('provides proper ARIA attributes for rows', () => {
    ...
    expect(row).toHaveAttribute('aria-label', expect.stringContaining(mockCategory.name));
  });

  it('includes ARIA expanded attributes for categories with children', () => {
    renderWithTableContext(
      <CategoryTableRow
        category={{ ...mockCategory, childCount: 2 }}
        showSiteColumn={false}
        onDeleteClick={mockDeleteClick}
      />
    );
    ...
    // Check if aria-expanded attribute exists, value might be true or false
    expect(row).toHaveAttribute('aria-expanded');
    expect(row).toHaveAttribute('aria-controls', expect.stringContaining(mockCategory.id));
  });
  ...
  it('has proper ARIA label for the delete button', () => {
    ...
    // Verify delete button has descriptive title or aria-label
    const deleteButton = screen.getByTestId(`delete-button-${mockCategory.id}`);
    expect(deleteButton).toHaveAccessibleName(expect.stringContaining('Delete'));
  });
  ...
  it('includes proper ARIA label for drag handle button', () => {
    ...
    // Find drag handle button by its accessible name
    const dragHandle = screen.getByRole('button', { name: /drag/i });
    expect(dragHandle).toBeInTheDocument();
    expect(dragHandle).toHaveAccessibleName('Drag to reorder');
  });
