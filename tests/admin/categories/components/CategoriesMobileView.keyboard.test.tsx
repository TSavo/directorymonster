...
describe('CategoriesMobileView Keyboard Accessibility Tests', () => {
  ...
  it('supports keyboard navigation for action links', async () => {
    ...
    await user.tab();
    expect(document.activeElement).toBe(screen.getByTestId('view-link-category_1'));
    ...
    // Tab to edit link
    await user.tab();
    expect(document.activeElement).toBe(screen.getByTestId('edit-link-category_1'));
  ...
  it('supports Space key activation for delete buttons', async () => {
    ...
    // Tab to the first delete button (3rd tab as it comes after view and edit links)
    await user.tab(); // View link
    await user.tab(); // Edit link
  ...
  it('can tab backwards through interactive elements', async () => {
    ...
    await user.tab(); // View link 1
    await user.tab(); // Edit link 1
    await user.tab(); // Delete button 1
    await user.tab(); // View link 2
    await user.tab(); // Edit link 2
    ...
    expect(document.activeElement).toBe(screen.getByTestId('edit-link-category_2'));
    
    await user.keyboard('{Shift>}{Tab}{/Shift}'); // Shift+Tab
    expect(document.activeElement).toBe(screen.getByTestId('view-link-category_2'));
