...
describe('useCategories Hook - Sorting', () => {
  ...
  it('should use order as default sort field in ascending order', () => {
    ...
    // First two categories have order 1 (category_1 and category_3), then category_2 with order 2
    expect(result.current.filteredCategories[0].order).toBe(1);
    expect(result.current.filteredCategories[1].order).toBe(1);
    expect(result.current.filteredCategories[2].order).toBe(2);
  });
  ...
  it('should handle sorting with pagination correctly', () => {
    ...
    // First page should have items in order
    expect(result.current.currentCategories.length).toBe(5);
    // Check the first and last items on the page
    const firstItem = result.current.currentCategories[0];
    const lastItem = result.current.currentCategories[4];
    expect(firstItem.name.includes('Test Category')).toBe(true);
    expect(lastItem.name.includes('Test Category')).toBe(true);
  });
