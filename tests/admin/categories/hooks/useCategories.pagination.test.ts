...
describe('useCategories Hook - Pagination', () => {
  ...
  it('should adjust current page when total pages decreases below current page', () => {
    ...
    const mockUseCategories = () => {
      ...
      const [categories, setCategories] = useState(paginatedCategories);
      const [totalPages, setTotalPages] = useState(3);

      // Simulate the effect that adjusts current page when total pages decreases
      useEffect(() => {
      ...
      return {
        ...
        currentCategories: categories.slice(0, 10),
        showHierarchy: false,
    ...
    const { result, rerender } = renderHook(() => mockUseCategories());

    // Verify initial state
    expect(result.current.currentPage).toBe(3);
    expect(result.current.totalPages).toBe(3);

    // Update to fewer categories (which would result in fewer pages)
    act(() => {
