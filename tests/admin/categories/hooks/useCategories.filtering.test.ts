...
describe('useCategories Hook - Filtering & Searching', () => {
  ...
  it('should search in name, metaDescription, and parentName fields', () => {
    ...
    expect(result.current.filteredCategories.length).toBe(2); // Matches both Test Category 1 and its child (due to parentName)
    expect(result.current.filteredCategories.some(c => c.name === 'Test Category 1')).toBe(true);
    ...
    // Should match both the parent category and its child (due to parentName)
  ...
  it('should apply multiple filters simultaneously', () => {
    ...
    result.current.filteredCategories.forEach(category => {
      expect(category.siteId).toBe('site_1');
      // Either name or parent name should include 'Test'
      expect(category.name.includes('Test') || (category.parentName && category.parentName.includes('Test'))).toBe(true);
    });
    ...
    // Should return one category (the child of category_1 that also has 'Test' in its parent name)
    expect(result.current.filteredCategories.length).toBe(1);
  });
  ...
  it('should maintain filter state after data is updated', async () => {
    ...
    // Now we should have 1 category with "Subcategory" in the name
    expect(result.current.filteredCategories.length).toBe(1);
    result.current.filteredCategories.forEach(category => {
